...
namespace VMSCore.WindowsForms
{
    ...
    public class EventDrivenTCPClient : IDisposable
    {
        ...
        public delegate void delDataReceived(EventDrivenTCPClient sender, byte[] data);
        ...
        public EventDrivenTCPClient(IPAddress ip, int port, bool autoreconnect )
        {
            this._IP = ip;
            this._Port = port;
        ...
        private void cbDataReceived(IAsyncResult result)
        {
            ...
            {
                String Chuoi = _encode.GetString(dataBuffer, 0, bytes);
                byte[] byData = new byte[Chuoi.Length];
                for (int i = 0; i < Chuoi.Length; i++)
                {
                    byData[i] = dataBuffer[i];
                }
                DataReceived.BeginInvoke(this, byData, new AsyncCallback(cbDataRecievedCallbackComplete), this);
            }
